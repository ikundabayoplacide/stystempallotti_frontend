...
function emptyItem(): StockRequestItemInput {
...
interface ItemRowProps {
  item: StockRequestItemInput;
  index: number;
  onChange: (index: number, field: keyof StockRequestItemInput, value: string | number) => void;
...
function RequestFormModal({ existing, onClose, onSaved }: RequestFormModalProps) {
  ...
  const handleItemChange = useCallback(
    (index: number, field: keyof StockRequestItemInput, value: string | number) => {
    ...
    <div className="fixed inset-0 bg-secondary-100/50 z-50 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl flex flex-col" style={{ maxHeight: "92vh" }}>
        <Card className="!p-0 overflow-hidden flex flex-col h-full">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-custom-300 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-bold text-secondary-100">
                {existing ? "Edit Request" : "New Stock Request"}
              </h3>
            </div>
            <button
              type="button"
          ...
          {/* Body */}
          <form id="stock-request-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto min-h-0">
...
export default function StockRequestsPage() {
  ...
  const myQuery  = useGetMyStockRequestsQuery(queryParams,  { skip: !isStockUser });
  const allQuery = useGetAllStockRequestsQuery(queryParams, { skip: !isReviewer });
  ...
  // KPI counts from loaded data (from the full list for summary)
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-custom-700 mt-1">
              {isStockUser ? "Manage your procurement requests" : "Review and action incoming stock requests"}
            </p>
          </div>
