...
interface RequestItem { stockItem: BindingStockItem; quantity: number }

function RequestModal({ stockItems, onClose, onSuccess }: {
  stockItems: BindingStockItem[];
  onClose: () => void;
  onSuccess: () => void;
}) {
  ...
  const [createSortie, { isLoading }] = useCreateBindingStockSortieMutation();
    ...
    <div className="fixed inset-0 bg-secondary-100/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <Card className="!p-6 max-w-xl w-full my-8">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="text-xl font-bold text-secondary-100">Request Binding Stock</h3>
            <p className="text-sm text-custom-700 mt-0.5">Select items and quantities to request</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="rounded-xl border border-custom-300 overflow-hidden">
              ...
              {items.map((item) => (
                <div key={item.stockItem.id} className="flex items-center gap-3 px-4 py-3 border-b border-custom-100 last:border-0">
                  ...
                  <button type="button" onClick={() => setItems((p) => p.filter((i) => i.stockItem.id !== item.stockItem.id))}
          ...
          <div>
            ...
            <textarea value={reason} onChange={(e) => setReason(e.target.value)}
              placeholder="e.g. Need items for binding department..." rows={2}
...
function EditModal({ sortie, onClose, onSuccess }: {
  sortie: BindingStockSortie;
  onClose: () => void;
  onSuccess: () => void;
}) {
  const [qty, setQty]       = useState(String(parseFloat(sortie.quantityOut) || 1));
  const [reason, setReason] = useState(sortie.reason ?? "");
  const [createSortie, { isLoading }] = useCreateBindingStockSortieMutation();

  // Re-submit as new (delete old is not available for receptionist; just create replacement)
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const q = parseInt(qty);
    if (!q || q <= 0) { toast.error("Enter a valid quantity"); return; }
    if (!reason.trim()) { toast.error("Please provide a reason"); return; }
    try {
      await createSortie({ stockItemId: sortie.stockItemId, quantityOut: q, reason: reason.trim() }).unwrap();
  ...
  return (
    <div className="fixed inset-0 bg-secondary-100/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <Card className="!p-6 max-w-md w-full my-8">
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-xl font-bold text-secondary-100">Edit Request</h3>
          <button onClick={onClose} className="text-custom-700 hover:text-secondary-100"><HiOutlineX className="w-6 h-6" /></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-secondary-100 mb-1">Item</label>
            <p className="text-sm font-semibold text-secondary-100 px-3 py-2 rounded-xl bg-custom-100">{sortie.stockItem?.itemName ?? "—"}</p>
          </div>
          <div>
...
export default function BindingStockRequestsPage() {
  const [showModal, setShowModal]   = useState(false);
  const [editTarget, setEditTarget] = useState<BindingStockSortie | undefined>(undefined);
  const [search, setSearch]         = useState("");

  const { data: stockData, isLoading: stockLoading, refetch: refetchStock } =
    useGetBindingStockItemsQuery({ limit: 200 });

  const { data: sortiesData, isLoading: sortiesLoading, refetch: refetchSorties } =
    useGetMyBindingStockSortiesQuery({ limit: 100 });
    ...
    <DashboardLayout notificationCount={pendingCount}>
      <div className="space-y-6 font-[family-name:var(--font-family-primary)]">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-custom-700 mt-0.5">Request items from binding stock</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* Available binding stock items */}
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-secondary-100 uppercase tracking-wide">Available Binding Stock Items</h2>
            ...
            {stockLoading ? (
              ...
              <Card className="!p-8 text-center">
                ...
                <p className="text-sm text-secondary-100 font-semibold">No binding stock items available</p>
                <p className="text-xs text-custom-700 mt-1">Ask the supervisor to add binding items</p>
              </Card>
            ) : (
      ...
      {editTarget && (
        <EditModal
          ...
          onSuccess={() => { setEditTarget(undefined); refetchSorties(); }}
        />
      )}
    </DashboardLayout>
  );
