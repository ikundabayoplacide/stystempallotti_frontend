...
export default function ReceptionMaterialRequestPage() {
  ...
  return (
    <DashboardLayout userRole="receptionist" userName={userName ?? "Receptionist"} notificationCount={0}>
              ...
              {badge != null && badge > 0 && (
                <span
                  className={`w-5 h-5 text-white text-[10px] font-bold rounded-full flex items-center justify-center ${
                    id === "my-requests" ? "bg-yellow-500" : "bg-primary-500"
                  }`}
                >
                  {badge > 9 ? "9+" : badge}
                </span>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-custom-100 border-b border-custom-300">
                  <tr>
                    {["Item Name", "Category", "Unit", "Stock", "Status", "Action"].map((h) => (
                      <th
                        key={h}
                        className={`px-3 py-2.5 text-xs font-bold text-secondary-100 uppercase ${
                ...
                <tbody className="divide-y divide-custom-200">
                  {itemsLoading ? (
                    <tr>
                      <td colSpan={6} className="px-4 py-8 text-center text-custom-700">Loading…</td>
                    </tr>
                        ...
                        <td className="px-3 py-2.5">
                          <span
                            className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${
                              stockStatusColors[item.stockStatus] ?? "bg-gray-100 text-gray-600"
                            }`}
                          >
                  ...
                  <Card key={s} className="!p-4 flex gap-2">
                    <div
                      className={`w-7 h-7 rounded-xl flex items-center justify-center mb-2 ${
                        cfg ? cfg.bgColor : "bg-primary-100"
                      }`}
                    >
                      {cfg ? (
                ...
                <table className="w-full">
                  <thead className="bg-custom-100 border-b border-custom-300">
                    <tr>
                      {["Item", "Qty", "Reason", "Status", "Date", ""].map((h) => (
                        <th key={h} className="px-3 py-2.5 text-left text-xs font-bold text-secondary-100 uppercase">
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className="divide-y divide-custom-200">
                    {sortiesLoading ? (
                      ...
                      <tr>
                        <td colSpan={6} className="px-4 py-8 text-center text-custom-700">No requests found</td>
                      </tr>
                    ) : (
                      filteredSorties.map((s) => {
                        ...
                        return (
                          <tr key={s.id} className="hover:bg-custom-50 transition-colors">
                            ...
                            <td className="px-3 py-2.5 text-xs text-custom-700 max-w-[160px] truncate">
                              {s.reason ?? "—"}
                            </td>
                            <td className="px-3 py-2.5">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span
                                  className={`text-xs font-bold px-2 py-0.5 rounded-full ${cfg.bgColor} ${cfg.color}`}
                                >
                                  {cfg.label}
                                </span>
                              </div>
                    ...
                    {items
                      ...
                      .map((i) => (
                        <option key={i.id} value={i.id}>
                          {i.itemName} ({i.currentStock} {i.unit} available)
                        </option>
                      ))}
                  </select>
                </div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-lg font-bold text-secondary-100">
                    {selectedSortie.stockItem?.itemName ?? "Material Request"}
                  </h3>
                  <span
                    className={`inline-block mt-1.5 text-xs font-bold px-3 py-1 rounded-full ${
                      statusConfig[selectedSortie.status].bgColor
                    } ${statusConfig[selectedSortie.status].color}`}
                  >
                    {statusConfig[selectedSortie.status].label}
                  </span>
                </div>
                <button onClick={() => setSelectedSortie(null)} className="text-custom-700 hover:text-secondary-100">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-secondary-100">
                    {parseFloat(selectedSortie.quantityOut)} {selectedSortie.stockItem?.unit ?? ""}
                  </span>
                </div>
                {selectedSortie.reason && (
                  <div className="flex justify-between gap-4">
                    <span className="text-custom-700 shrink-0">Reason</span>
                    <span className="text-secondary-100 text-right">{selectedSortie.reason}</span>
                  </div>
                )}
                {selectedSortie.notes && (
                  <div className="flex justify-between gap-4">
                    <span className="text-custom-700 shrink-0">Notes</span>
                    <span className="text-secondary-100 text-right">{selectedSortie.notes}</span>
                  </div>
                )}
                {selectedSortie.approvedBy && (
                  <div className="flex justify-between">
                    <span className="text-custom-700">
                      {selectedSortie.status === "approved" ? "Approved by" : "Reviewed by"}
                    </span>
                    <span className="font-semibold text-secondary-100">{selectedSortie.approvedBy.name}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-custom-700">Requested</span>
                  <span className="text-secondary-100">{new Date(selectedSortie.createdAt).toLocaleString()}</span>
                </div>
              </div>
