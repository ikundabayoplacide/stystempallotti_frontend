...
function downloadProforma(q: Proforma) {
  import("jspdf").then(({ jsPDF }) => {
    ...
    doc.setFontSize(9);
    doc.text("Proforma / Proposal", pageW - margin, 9.5, { align: "right" });

    y = 24;

    // ── Proforma number + status ────────────────────────────────────────────
    doc.setTextColor(30, 30, 30);
    doc.setFontSize(18);
    doc.setFont("helvetica", "bold");
    doc.text(q.proformaNo, margin, y);
      ...
      <Card className="!p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-6 pt-4 border-t border-custom-300">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => downloadProforma(quotation)}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-custom-300 text-custom-700 hover:bg-custom-100 transition-colors text-sm font-semibold"
            >
      ...
      <div className="space-y-6 font-[family-name:var(--font-family-primary)]">
        ...
        <Card className="!p-4">
          <div className="flex flex-col md:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search by proforma #, job title, or customer…"
                value={search}
        ...
        {isLoading ? (
          ...
          <Card className="!p-12 text-center">
            ...
            <p className="text-sm text-custom-700 mt-1">Proformas are auto-created when a job is created</p>
          </Card>
        ) : (
          <Card className="!p-0 overflow-hidden">
